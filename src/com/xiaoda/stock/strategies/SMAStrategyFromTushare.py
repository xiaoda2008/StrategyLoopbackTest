...
class SMAStrategyFromTushare(StrategyParent):
    '''
    SMA（简单移动平均）策略
...
#可参考的文章：https://www.jianshu.com/p/642ad8a0366e
    ...
    def getShareToBuyOrSell(self,priceNow,latestDealPrice, 
        ...
        yesterdayDayMA20 = stock_data.at[todayDate,'yesterday_MA20']
        

        #需要调整，当天，只可能知道当天开盘价，无法知道当天平均价，不能采用上帝模式

        #应该用当天开盘价与前一天的MA20进行比较
        if stock_data.at[todayDate,'pre_close']<yesterdayDayMA20 and priceNow>yesterdayDayMA20:
            #前一天收盘价格低于20日均线，且当天开盘价格高于20日均线-》上穿20日均线，可以买入
            return math.floor(nShare/2)
        elif stock_data.at[todayDate,'pre_close']>yesterdayDayMA20 and priceNow<yesterdayDayMA20:
