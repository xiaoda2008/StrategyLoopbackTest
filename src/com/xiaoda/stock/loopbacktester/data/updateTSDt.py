...
for index,stockCode in stockCodeList.items():
    ...
    try:
        sk.to_sql(name='s_k_data', con=mysqlEngine, chunksize=1000, if_exists='append', index=None)
    except sqlalchemy.exc.IntegrityError:
        log.logger.warning("此前处理过该股票%s的kdata"%(stockCode))
...
for index,stockCode in stockCodeList.items():
    ...
    #获取该股票数据并写入数据库
    #sk=tushare.pro_bar(ts_code=stockCode, start_date=startday, end_date=endday)
    db=sdDataAPI.daily_basic(ts_code=stockCode,start_date=startday,end_date=endday)
            
    if db.empty:
        #如果没有任何返回值，说明该时间段内该股票没有交易数据
        log.logger.warning('%s在%s到%s时段内无每日基础数据'%(stockCode,startday,endday))
        continue
    ...
    try:
        db.to_sql(name='s_daily_basic', con=mysqlEngine, chunksize=1000, if_exists='append', index=None)
    except sqlalchemy.exc.IntegrityError:
        log.logger.warning("此前处理过该股票%s的daily_basic"%(stockCode))    
    
    log.logger.info('处理完股票%s在%s到%s区间内的每日基础数据'%(stockCode,startday,endday))
...
for index,stockCode in stockCodeList.items():
    ...
    try:
        ad.to_sql(name='s_adj_data',con=mysqlEngine,chunksize=1000,if_exists='append',index=None)
    except sqlalchemy.exc.IntegrityError:
        log.logger.warning("此前处理过该股票%s的复权因子"%(stockCode))    
