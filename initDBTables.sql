...
-- 这个是自然日时间
insert into u_dataupdatelog (content_name,content,comments) values ('last_totally_update_time','','the time of last total data update');

-- 数据范围起始日期，在完成所有数据更新任务后更新该字段
-- 这个日期，应当是第一个交易日的日期，不是自然日日期
insert into u_dataupdatelog (content_name,content,comments) values ('data_start_dealday','','the start deal day(include) of all data');

-- 数据范围结束日期，在完成所有数据更新任务后更新该字段
-- 这个日期，应当是最后一个交易日的日期，不是自然日日期
insert into u_dataupdatelog (content_name,content,comments) values ('data_end_dealday','','the end deal day(include) of all data');

-- 最近一次数据更新（不论是否完成完整的数据更新），在每个数据更新后都更新该字段
-- 这个是自然日时间
insert into u_dataupdatelog (content_name,content,comments) values ('last_update_time','','the time of last data update');
